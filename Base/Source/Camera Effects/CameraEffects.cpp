...
void CCameraEffects::RenderUI()
{
	...
	modelStack.Translate(4.3f, -4.3f, 0);*/

	/*if (CPlayerInfo::GetInstance()->GetWeapon() == 0)
	{
	RenderHelper::RenderMesh(pistolicon);
	}
	else if (CPlayerInfo::GetInstance()->GetWeapon() == 1)
	{
	RenderHelper::RenderMesh(ARicon);
	}*/
	...
	modelStack.PushMatrix();
	// Translate the current transformation
	modelStack.Translate(position.x + 120, position.y - 150, position.z);
	// Scale the current transformation
	modelStack.Scale(scale.x - 245, scale.y - 245, scale.z - 245);
